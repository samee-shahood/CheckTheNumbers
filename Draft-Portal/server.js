...
var useTeamSchema = new Schema({
  name: String,
  team: Array,
}, {collection: 'team-data'});

var TeamData = mongoose.model('Team', useTeamSchema);
...
io.on('connection', function(client) {  
  ...
  client.emit('getTeamData', UserDataArray); //send user info


  client.on('sendTeamData', function(teamarray, username){
    var newTeam = new TeamData();

    newTeam.name = username;

    newTeam.team = teamarray;
  ...
  client.on('login', function(data, username){
    ...
    if (players.length == 4){ //if 4 users joined
