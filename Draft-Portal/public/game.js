...
socket.on('order', function(users){
    $('.order').append($('<div class="draftorder">Draft Order</div>'));
...
socket.on('getTeamData', function(info){
    teamArrays = info;
})
    ...
    for (var i = 0; i < userInfo['items'].length; i++){
        if (userInfo.items[i].username == document.getElementById("username").value){
            ...
            if (userInfo.items[i].password == document.getElementById("password").value){
              ...
              //  for (var j = 0; j < teamArrays['items'].length; j++){
                //    if (teamArray['items'].name == username){
                  //      verify == false;

                    //    var rem = ("#2");
                    //    rem.remove();
                      //  $('.playerList').append($('<div id="'+ 2 +'"><div class="message">You have already drafted a team!</div></div>'));

                      //  window.location.replace("http://localhost:4200/postdraftleaguehome");
                        //j = (teamArrays['items'].length + 5);
                   // }
                ///}
...
socket.on('showteam', function(id){
    ...
    $.ajax({
        ...
        success: function(copyrightAndStats) {
          $.each(copyrightAndStats, function (index, player) {
            if (index == "people") {
                $('.teamList').append($('<h1 class="tem" name="team">'+player[0].fullName+'</h1>'));
            }
          });
        }
    }); 
    ...
    if (team.length == 4){
        socket.emit('sendTeamData', team, username);
        redirect();
    }
...
function createPlayerList(){
    ...
    var removedupe1 = $(".draftorder");
    removedupe1.remove();

    $('.teamList').append($('<div class="draftorder">YOUR TEAM</div>'));

    $('.drafted').append($('<div class="draftorder">Draft Results</div>'));
...
socket.on('displayPick', function(id, client){
    $.ajax({
        ...
        success: function(copyrightAndStats) {
          $.each(copyrightAndStats, function (index, player) {
            if (index == "people") {
                $('.drafted').append($('<h1 class="draff">'+client+' drafts '+player[0].fullName+'</h1>'));
            }
          });
        }
    });            
